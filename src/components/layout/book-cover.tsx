...
export const BookCover = memo(function BookCover({ book, size = 'md' }: BookCoverProps) {
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-md text-white shadow-sm',
        hasCoverImage ? 'bg-muted' : ['bg-gradient-to-br', gradientById(book.id)],
        isSmall ? 'h-20 w-14' : 'h-44 w-full',
      )}
      aria-hidden="true"
    >
      {hasCoverImage ? (
        <img src={coverImageData} alt="" className="h-full w-full object-cover" loading="lazy" />
      ) : (
        <>
